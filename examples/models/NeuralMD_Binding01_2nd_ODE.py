...
class NeuralMD_Binding01(nn.Module):
    def __init__(self, args):
        super(NeuralMD_Binding01, self).__init__()
        ...
        if self.velocity_refined_value_coefficient > 0:
            self.refined_velocity_model = FrameNetLigand01(
                hidden_channels=args.emb_dim,
                cutoff=args.FrameNet_cutoff,
                num_layers=args.FrameNet_num_layers,
                num_radial=args.FrameNet_num_radial,
    ...
    def forward(self, t, input_data, condition):
        ...
        # [num_atom, 3]
        acceleration = ligand_vec / ligand_mass.unsqueeze(1)

        if self.velocity_refined_value_coefficient > 0:
            _, velocity_refined_value = self.refined_velocity_model(
                z=condition[0],
                pos=ligand_positions,
                batch=condition[1],
            )
            velocity = velocity + self.velocity_refined_value_coefficient * velocity_refined_value
