...
class ComplexFrameLayer(nn.Module):
    def __init__(
        ...
        self.scalarization_linear = nn.Sequential(
    ...
    def forward(self,
        ...
        # [num_edge, latent_dim]
        h_edge_weight = (self.scalarization_linear(torch.permute(h_edge_scalarization, (0, 2, 1))) + torch.permute(h_edge_scalarization, (0, 2, 1))[:, :,0].unsqueeze(2)).squeeze(-1)

        dim_size = pos_ligand.shape[0]
        # [num_ligand, latent_dim]
        ligand_repr = scatter(h_edge_weight, edge_ligand_idx, dim=0, dim_size=dim_size)
        return ligand_repr


class FrameNetComplexEnergy01(nn.Module):
    def __init__(
        self,
        ...
        super(FrameNetComplexEnergy01, self).__init__()
        self.latent_dim = latent_dim
    ...
    def forward(self, ligand_repr, ligand_vec_input, pos_ligand, batch_ligand, residue_repr, pos_residue, batch_residue):
        ...
        for layer_idx, layer in enumerate(self.complex_frame_layers):
            dligand_repr = layer(
                h_ligand=h_ligand, pos_ligand=pos_ligand, batch_ligand=batch_ligand, edge_ligand_idx=edge_ligand_idx, ligand_vec=ligand_vec_input,
            ...
            h_ligand = h_ligand + dligand_repr
